/*****************************************************
 * FindClosestNeighbor                               *
 * Returns the position in the vector of the masses  *
 ...
 *  LookAtPos         is the coordinates where the camera is looking/facing                  *
						...
						const glm::vec3 cameraPos, const glm::vec3 LookAtPos, const int clippingPlaneNear, const float maxSpringLength)
{
    ...
    for(int i = 0; i < massPositions.size(); i++)
    {
		...
		float rayRadius = maxSpringLength;
        if(length < rayRadius)
        {
            //info to be saved
