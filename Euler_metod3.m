...
for tid = 0:h:slutTid
    ...
    for j = 1:bredd*hojd  
            ...
            if j > b && mod(j,b) ~= 1
                ...
                %if we are 2 steps in from the top and left edges
                if j > 2b && mod(j,b) > 2
                    k2Upp = ((particle_old(:,j-2b)-particle_old(:,j)).*(abs(norm(particle_old(:,j-2b)-particle_old(:,j))-oa)/norm(particle_old(:,j-2b)-particle_old(:,j))));
                    c2Upp = 0;
                    k2Vanster = ((particle_old(:,j-2)-particle_old(:,j)).*(abs(norm(particle_old(:,j-2)-particle_old(:,j))-oa)/norm(particle_old(:,j-2)-particle_old(:,j))));
                    c2Vanster = 0;
                else
                    k2Upp = 0;
                    c2Upp = 0;
